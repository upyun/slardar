...
local function _do_connect(host, port)
    local ok, err, sock
...
local function _connect(host, port)
    ...
    while retry_connect <= max_retry_times do
        sock, err = _do_connect(host, port)
...
local function _do_flush()
    ...
    local sock, err = checkups.ready_ok(ups_name, _connect)
    if not sock then
        return nil, err
    end
...
function _M.init(user_config)
    ...
    if not ups_name then
        return nil, "no upstream name specified"
    end
